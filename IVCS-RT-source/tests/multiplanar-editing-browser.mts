...
try{
 ...
 const d=createDemoRadiotherapyDataset(),roi={...d.initialRois[0],id:'test-edit',name:'Editing test',locked:false,visible:true,sliceMasks:{}};
 ...
 async function saved(){await page.locator('#btn-export-menu').click();const download=page.waitForEvent('download');await page.locator('#btn-export-json').click();const f=await download;return readSessionFile(new Blob([await fs.readFile((await f.path())!)]));}
 ...
 assert.ok(Number(await popup.getByTestId('editable-axial').getAttribute('data-zoom'))>1,'popup must fit after styles load');
 ...
 await popup.screenshot({path:'build/detached-editing-20260923.png'});
 ...
 await page.screenshot({path:'build/multiplanar-editing-20260923.png'});
 ...
 await page.locator('#tab-operations').click();await page.getByTestId('open-cleanup').click();await page.getByTestId('cleanup-preview').click();await page.getByTestId('cleanup-summary').waitFor({timeout:30000});await page.screenshot({path:'build/cleanup-preview-20260923.png'});
