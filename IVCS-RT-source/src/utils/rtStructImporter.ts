import {maskGeometry,maskScale,roiAtScale,type MaskScale} from './segmentationGrid';
...
export function importRtStruct(bytes:Uint8Array,series:DicomSeries,scale:MaskScale=1):StructureRoi[]{
  ...
  return definitions.map((definition:any)=>{
    ...
    for(const contour of items(entry || {elements:{}},'x30060040')){
      ...
      const native=series.slices.find(s=>s.sopInstanceUID===references[0].string('x00081155'));
      if(!native)throw new Error('RTSTRUCT referencia imágenes que no están en la serie abierta.');
      const slice=maskGeometry(native,scale);
    ...
    const sliceMasks={};for(const [z,loops] of bySlice){const slice=maskGeometry(series.slices[z],scale);sliceMasks[z]=rasterizeLoops(loops,slice.rows,slice.cols,kind==='CLOSEDPLANAR_XOR');}
    ...
    return {maskScale:scale,id:'import-'+number+'-'+crypto.randomUUID(),name:dicomText(definition,'x30060026',ds.string('x00080005')) || 'ROI '+number,type,color:'#'+color.map((c:number)=>Math.max(0,Math.min(255,c)).toString(16).padStart(2,'0')).join(''),visible:true,locked:false,opacity:.4,sliceMasks};
  });
}
export function compareContours(original:StructureRoi[],restored:StructureRoi[],series:DicomSeries){
  return original.map((source,index)=>{
    const scale=Math.max(maskScale(source),maskScale(restored[index])) as MaskScale,roi=roiAtScale(source,series,scale);
    const other=restored[index]?roiAtScale(restored[index],series,scale):undefined;let before=0,after=0,common=0,volumeBefore=0,volumeAfter=0,maxBoundaryDistanceMm=0;const changedSlices:number[]=[];
    for(const native of series.slices){const slice=maskGeometry(native,scale);const a=roi.sliceMasks[slice.sliceIndex],b=other?.sliceMasks[slice.sliceIndex];let changed=false,ca=0,cb=0;
