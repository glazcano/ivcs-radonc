...
export function serializeSession(session: Session): string {
  ...
  return JSON.stringify(portable, (_, value) => value instanceof Int16Array || value instanceof Uint8Array ? Array.from(value) : value);
}
...
export function parseSession(text: string): Session {
  ...
  const restoreSlices = (series: DicomSeries) => {
    if (!series || !Array.isArray(series.slices)) throw new Error('Sesión sin imágenes.');
    series.slices.forEach((slice,i) => {
      if (!Number.isInteger(slice.rows) || !Number.isInteger(slice.cols) || slice.rows <= 0 || slice.cols <= 0 || slice.sliceIndex !== i || !Array.isArray(slice.huData) || slice.huData.length !== slice.rows*slice.cols || !slice.huData.every(v=>Number.isInteger(v) && v >= -32768 && v <= 32767)) throw new Error('Imágenes inválidas en la sesión.');
      slice.huData = new Int16Array(slice.huData);
    });
    validateVolume(series.slices);
  };
  restoreSlices(s.series);
  s.studies.forEach(restoreSlices);
